  ...
  if (warnings.length === 0) {
    return (
      <div className="p-4 text-center">
        ...
        <p className="text-sm text-[var(--pf-text-secondary)]">No warnings detected</p>
        <p className="text-xs text-[var(--pf-text-muted)] mt-1">Your system architecture looks good!</p>
      </div>
    );
  }
...
function WarningItem({ warning }: WarningItemProps) {
  ...
  const color = isError ? 'var(--pf-error)' : 'var(--pf-warning)';
