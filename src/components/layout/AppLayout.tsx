...
export function AppLayout() {
	...
	// Using useEffect with cleanup is the correct pattern for DOM side effects
	useEffect(() => {
		if (isMenuOpen) {
	...
	return (
		<>
			<div className='flex flex-col min-h-screen bg-[var(--pf-bg-primary)]'>
					...
					<div className='mx-auto flex h-20 items-center justify-between px-6 lg:px-8'>
						...
						<Link
							...
							<h1
								...
								style={{
									background:
										'linear-gradient(135deg, #60A5FA, #34D399, #FDE047, #F472B6)',
						...
						<div className='flex items-center gap-6'>
							...
							<nav className='hidden md:flex items-center gap-8'>
								{navItems.map((item) => {
									const isActive =
										item.isInternal && location.pathname === item.href;
				...
				<main className='flex-1 flex flex-col pt-20 overflow-hidden min-h-0'>
					<Outlet />
				</main>
			</div>
